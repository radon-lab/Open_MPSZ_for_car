...
#define IDLE_OFF_RPM 500 //минимальное значение признака ХХ (обр*мин)
...
const int8_t UOZ_rpm_table [100] PROGMEM = { //значениея УОЗ в зависимости от оборотов двигателя 1яч - 100обр*мин (0..9900)(обр*мин), значиния в ячейке (-15..75)(°)
...
const int8_t UOZ_temp_table [165] PROGMEM = { //значениея УОЗ в зависимости от температуры 1яч - 1°с (-45..120)(°с), значиния в ячейке (-15..75)(°)
...
const uint16_t _def_deg_dellay = (1000000 / ((float)IDLE_OFF_RPM / 60.00) / 32.00) / 90.00 * IDLE_UOZ_DEFAULT; //угол опережения зажигания по умолчанию
...
int main(void)  //инициализация
{
  ...
  _deg_dellay = _def_deg_dellay; //устанавоиваем угол опережения по умолчанию
  ...
  for (;;) //главная
  {
    ...
    if (eng_flag) { //если зажигание активно
      ...
      uint8_t UOZ_now = ((int8_t)75 - pgm_read_byte(&UOZ_rpm_table[rev_per_min / 100])) + ((int8_t)75 - pgm_read_byte(&UOZ_temp_table[temp])) + _idle_uoz_correct; //рассчитываем текущий УОЗ
    ...
    if (_micros == 65535) {
      _deg_dellay = _def_deg_dellay; //если двигатель заглох, устанавливаем УОЗ по умолчанию
